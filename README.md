...
```bash
llmschema -d docs/db-schema --no-database-info
```

**Preserve Table Files From Previous Multi-file Runs**
```bash
llmschema -d docs/db-schema --preserve-stale-files
```
...
| `--no-database-info` | | Exclude database type and version from the output | `false` |
...
Database schema documentation is in `schema.md`. Read it when working on
...
Single-file output includes the database type and version followed by every
table, its columns, indexes, and relationships in one Markdown document:
