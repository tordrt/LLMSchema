...
func init() {
	...
	rootCmd.Flags().BoolVar(&preserveStaleFiles, "preserve-stale-files", false, "Do not delete table files generated by previous runs")
	if err := rootCmd.MarkFlagRequired("db-url"); err != nil {
		panic(err)
	}
...
func formatOutput(extractedSchema *schema.Schema) (err error) {
	...
	if outputDir != "" {
		...
		multiFormatter.PreserveStaleFiles = preserveStaleFiles
