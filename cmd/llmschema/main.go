...
func newRootCmd(extractAndFormat extractAndFormatFunc) *cobra.Command {
	...
	cmd.Flags().StringVar(&opts.dbURL, "db-url", "", "Database connection string (postgres://, mysql://, or sqlite://)")
	...
	if err := cmd.MarkFlagRequired("db-url"); err != nil {
		panic(err)
	}
...
func (opts *cliOptions) run(cmd *cobra.Command, extractAndFormat extractAndFormatFunc) (err error) {
	...
	return extractAndFormat(cmd.Context(), opts.dbURL, extractionOpts, outOpts)
}
